...
import { TaskExecutor, UnsuccessfulWorkflowExecution } from '@angular-devkit/schematics';
import { PhpPackageTaskFactoryOptions, PhpPackageTaskOptions } from './options';

type PhpPackageManagerProfile = {
...
export class UnknownPhpPackageManagerException extends BaseException {
  constructor(name: string) {
    super(`Unknown php package manager "${name}".`);
  }
}
...
export default function(factoryOptions: PhpPackageTaskFactoryOptions = {}): TaskExecutor<PhpPackageTaskOptions> {
  ...
  return (options: PhpPackageTaskOptions) => {
    ...
    if (factoryOptions.allowPackageManagerOverride && options.packageManager) {
      ...
      if (!taskPackageManagerProfile) {
        throw new UnknownPhpPackageManagerException(options.packageManager);
      }
    ...
    return new Observable(obs => {
      const spinner = ora({
        text: 'Installing PHP packages...',
        // Workaround for https://github.com/sindresorhus/ora/issues/136.
        discardStdin: process.platform != 'win32'
      }).start();
      const childProcess = spawn(taskPackageManagerName, args, spawnOptions).on('close', (code: number) => {
        if (code === 0) {
          spinner.succeed('PHP Packages installed successfully.');
          spinner.stop();
          obs.next();
          obs.complete();
        } else {
      ...
      if (options.hideOutput) {
        childProcess.stdout.on('data', (data: Buffer) => bufferedOutput.push({ stream: process.stdout, data: data }));
        childProcess.stderr.on('data', (data: Buffer) => bufferedOutput.push({ stream: process.stderr, data: data }));
      }
    });
  };
}
